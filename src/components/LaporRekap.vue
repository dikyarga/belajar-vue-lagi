<template>
  <v-card>
    ...
    <v-data-table v-bind:headers="headers" v-bind:items="items" v-bind:search="search" :loading="loading">
      ...
      <td class="text-xs-right">{{ props.item.totalhargabeli }}</td>
      <td class="text-xs-right">{{ props.item.totalhargajual }}</td>
      <td class="text-xs-right">{{ props.item.totalstock }}</td>
      </template>
    </v-data-table>
  </v-card>
</template>
...
export default {
  ...
  data() {
    return {
      loading: true,
      pageTitle: 'Laporan Rekap Produk',
  ...
  methods: {
    loadData(hostAPI) {
      ...
      return new Promise((resolve, reject) => {
        ...
        axios({
          method: 'get',
          url: hostAPI + '/rekap_rest'
        }).then(res => {
          console.log('isi data : ', res.data.datarekappenjualan);
          if (this.items.length === 0) {
            this.items = res.data.datarekappenjualan
            this.successFetchServer1 = true
          } else {
            self.items = self.items.concat(res.data.datarekappenjualan)
