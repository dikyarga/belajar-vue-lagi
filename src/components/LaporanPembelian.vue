<template>
  <v-card>
    ...
    <v-data-table v-bind:headers="headers" v-bind:items="items" v-bind:search="search" :loading="loading">
      ...
      <td class="text-xs-right">{{ props.item.NoBeli }}</td>
      ...
      <td class="text-xs-right">{{ props.item.hargabeli }}</td>
      <td class="text-xs-right">{{ props.item.totalbeli }}</td>
      <td class="text-xs-right">{{ props.item.tanggalbeli }}</td>
      </template>
    </v-data-table>
  </v-card>
</template>
  ...
  methods: {
    loadData(hostAPI) {
      ...
      return new Promise((resolve, reject) => {
        ...
        axios({
          method: 'get',
          url: hostAPI + '/laporbeli_rest'
        }).then(res => {
          console.log('isi data : ', res.data.datalaporanpembelian);
          if (this.items.length === 0) {
            this.items = res.data.datalaporanpembelian
            this.successFetchServer1 = true
          } else {
            self.items = self.items.concat(res.data.datalaporanpembelian)
