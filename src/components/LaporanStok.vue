<template>
  <v-card>
    ...
    <v-data-table v-bind:headers="headers" v-bind:items="items" v-bind:search="search" :loading="loading">
      ...
      <td class="text-xs-right">{{ props.item.expireddate }}</td>
      </template>
    </v-data-table>
  </v-card>
</template>
  ...
  methods: {
    loadData(hostAPI) {
      ...
      return new Promise((resolve, reject) => {
        ...
        axios({
          method: 'get',
          url: hostAPI + '/stok_rest'
        }).then(res => {
          console.log('isi data : ', res.data.datastokpenjualan);
          if (this.items.length === 0) {
            this.items = res.data.datastokpenjualan
            this.successFetchServer1 = true
          } else {
            self.items = self.items.concat(res.data.datastokpenjualan)
